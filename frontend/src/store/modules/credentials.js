...
const mutations = {
	ADD_CREDENTIALS(state, credentials) {
		function _getCredentialIndex(credentialId) {
			for (let index = 0; index < state.credentials.length; index++) {
		...
		credentials.forEach(credential => {
			const stateCredentialIndex = _getCredentialIndex(credential.id)
			if (stateCredentialIndex !== undefined) {
				state.credentials.splice(stateCredentialIndex, 1, credential)
			}
			else {
